...
public sealed class BotRecoveryService<T> : IDisposable where T : class, IConsoleBotConfig
{
    ...
    private static void ValidateConfiguration(RecoveryConfiguration config)
    {
        if (config.MaxRecoveryAttempts < 1)
            throw new ArgumentException("MaxRecoveryAttempts debe ser al menos 1", nameof(config));
        if (config.InitialRecoveryDelaySeconds < 0)
            throw new ArgumentException("InitialRecoveryDelaySeconds no puede ser negativo", nameof(config));
        if (config.BackoffMultiplier < 1.0)
            throw new ArgumentException("BackoffMultiplier debe ser al menos 1.0", nameof(config));
    }
    ...
    private async Task MonitorBotsAsync(CancellationToken cancellationToken)
    {
        while (!cancellationToken.IsCancellationRequested)
        {
            ...
            {
                LogUtil.LogError($"Error en el monitor de recuperación del bot: {ex.Message}", "Recuperación");
            }
        }
    }
            ...
            foreach (var bot in _runner.Bots)
            {
                ...
                else if (bot.IsRunning && state.ConsecutiveFailures > 0)
                {
                    ...
                    if (uptime.TotalSeconds >= _config.MinimumStableUptimeSeconds)
                    {
                        ...
                        LogUtil.LogInfo("Recuperación", $"El bot {botName} ha estado estable durante {uptime.TotalMinutes:F1} minutos. Restableciendo los intentos de recuperación.");
                    }
                }
            }
        }
    ...
    private bool ShouldAttemptRecovery(BotSource<T> bot, BotRecoveryState state)
    {
        ...
        {
            LogUtil.LogError($"El bot {botName} ha excedido el número máximo de intentos de recuperación ({_config.MaxRecoveryAttempts})", "Recuperación");
            return false;
        }
        ...
        {
            LogUtil.LogError($"El bot {botName} se ha bloqueado {state.CrashHistory.Count} veces en los últimos {_config.CrashHistoryWindowMinutes} minutos. Deshabilitando la recuperación.", "Recuperación");
            return false;
        }
    ...
    private async Task AttemptRecovery(BotSource<T> bot, BotRecoveryState state, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            LogUtil.LogInfo($"Intentando recuperación para el bot {botName} (intento {state.ConsecutiveFailures}/{_config.MaxRecoveryAttempts})", "Recuperación");
            ...
            await Task.Run(() =>
            {
                ...
                {
                    LogUtil.LogError($"No se pudo iniciar el bot {botName}: {ex.Message}", "Recuperación");
                    throw;
                }
            ...
            if (bot.IsRunning)
            {
                LogUtil.LogInfo($"El bot {botName} se ha recuperado correctamente", "Recuperación");
            ...
            {
                throw new Exception("El bot se detuvo inmediatamente después de reiniciarse");
            }
        }
        catch (Exception ex)
        {
            LogUtil.LogError($"La recuperación falló para el bot {botName}: {ex.Message}", "Recuperación");
    ...
    public void EnableRecovery()
    {
        ...
        LogUtil.LogInfo("Servicio de recuperación del bot habilitado", "Recuperación");
    }

    /// <summary>
    /// Deshabilita el servicio de recuperación.
    /// </summary>
    public void DisableRecovery()
    {
        _config.EnableRecovery = false;
        LogUtil.LogInfo("Servicio de recuperación del bot deshabilitado", "Recuperación");
    }
}
