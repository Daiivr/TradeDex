...
public class RecoverableBotRunner<T> : BotRunner<T> where T : class, IConsoleBotConfig
{
    ...
    public void InitializeRecovery(RecoveryConfiguration config)
    {
        ...
        LogUtil.LogInfo("Servicio de recuperación del bot inicializado", "Recuperación");
    ...
    public override void Add(RoutineExecutor<T> bot)
    {
        if (Bots.Any(z => z.Bot.Connection.Equals(bot.Connection)))
            throw new ArgumentException($"{nameof(bot.Connection)} ya ha sido agregado.");

        // Crear una fuente de bot recuperable si la recuperación está habilitada
        if (_recoveryService != null)
        {
        ...
        {
            // Alternativa: fuente de bot regular
    ...
    {
        LogUtil.LogError($"El bot {e.BotName} se bloqueó a las {e.CrashTime:yyyy-MM-dd HH:mm:ss}", "Recuperación");
    }

    private void OnRecoveryAttempted(object? sender, BotRecoveryEventArgs e)
    {
        LogUtil.LogInfo($"Intentando recuperación para {e.BotName} (intento {e.AttemptNumber})", "Recuperación");
    }

    private void OnRecoverySucceeded(object? sender, BotRecoveryEventArgs e)
    {
        LogUtil.LogInfo($"Recuperación exitosa de {e.BotName} después de {e.AttemptNumber} intento(s)", "Recuperación");
    }

    private void OnRecoveryFailed(object? sender, BotRecoveryEventArgs e)
    {
        LogUtil.LogError($"No se pudo recuperar {e.BotName} después de {e.AttemptNumber} intentos. Motivo: {e.FailureReason}", "Recuperación");
    }
