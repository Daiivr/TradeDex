    ...
    private async Task MonitorBotExecutionAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (!_isIntentionallyStopped && !IsStopping && !cancellationToken.IsCancellationRequested)
            {
                ...
                LogUtil.LogError($"¡El bot {Bot.Connection.Name} se ha bloqueado inesperadamente!", "Recuperación");

                // El servicio de recuperación manejará el reinicio si está habilitado
            }
        }
        catch (OperationCanceledException)
        {
            // Esperado cuando se detiene la monitorización
        }
        catch (Exception ex)
        {
            LogUtil.LogError($"Error en la monitorización del bot {Bot.Connection.Name}: {ex.Message}", "Recuperación");
        }
    }
