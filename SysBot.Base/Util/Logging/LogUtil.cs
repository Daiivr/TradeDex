...
public static class LogUtil
{
    ...
    public static Func<string, string>? MessageLocalizer { get; set; }
    ...
    public static void LogSafe(Exception exception, string identity)
    {
        ...
        if (LogConfig.EnableMasterLog)
        {
            Logger.Log(LogLevel.Error, Localize($"Excepción desde {identity}:"));
            Logger.Log(LogLevel.Error, exception);
        }

        // Registrar en el log por bot
        if (LogConfig.EnablePerBotLogging)
        {
            var botLogger = GetOrCreateBotLogger(identity);
            botLogger.Log(LogLevel.Error, Localize("Ocurrió una excepción:"));
    ...
    public static void LogText(string message) => Logger.Log(LogLevel.Info, Localize(message));

    public static string LocalizeMessage(string message) => Localize(message);
    ...
    private static void Log(string message, string identity)
    {
        ...
        foreach (var fwd in Forwarders)
        {
            ...
            {
                Logger.Log(LogLevel.Error, Localize($"No se pudo reenviar el log desde {identity} - {message}"));
    ...
    private static string Localize(string message) => MessageLocalizer?.Invoke(message) ?? message;
}
