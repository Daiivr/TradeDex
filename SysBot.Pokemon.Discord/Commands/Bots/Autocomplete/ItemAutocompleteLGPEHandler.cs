...
public class ItemAutocompleteLGPEHandler : AutocompleteHandler
{
        ...
        try
        {
            ...
            var itemNames = strings.Item
                ...
                .Where(item =>
                    ...
                    !item.Name.Contains("???") &&
                    ItemRestrictions.IsHeldItemAllowed((ushort)item.Index, EntityContext.Gen7b))
                .ToList();
            ...
            return Task.FromResult(results.Any() ? AutocompletionResult.FromSuccess(results) : AutocompletionResult.FromSuccess(new[] { new AutocompleteResult(SysBot.Pokemon.Localization.AppLocalization.Get(SysBot.Pokemon.Localization.LocalizationKeys.DiscordAutocompleteNoMatches), "None") }));
        }
        catch (Exception ex)
        {
