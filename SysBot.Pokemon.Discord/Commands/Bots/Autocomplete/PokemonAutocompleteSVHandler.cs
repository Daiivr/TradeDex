...
/// <summary>
/// Autocomplete handler for Scarlet/Violet Pokemon species
/// </summary>
public class PokemonAutocompleteSVHandler : AutocompleteHandler
{
        ...
        IServiceProvider services)
    {
        try
        {
            ...
            // Get valid Pokemon for Scarlet/Violet
            var validSpecies = GetValidSpeciesForGame(GameVersion.SV);
            ...
            return Task.FromResult(
                results.Any()
                    ...
                    {
                        new AutocompleteResult(SysBot.Pokemon.Localization.AppLocalization.Get(SysBot.Pokemon.Localization.LocalizationKeys.DiscordAutocompleteNoMatches), "None")
                    })
            );
        }
        catch (Exception ex)
    ...
    private static List<(string Display, string Value)> GetValidSpeciesForGame(GameVersion game)
    {
        ...
        for (ushort species = 1; species < (ushort)Species.MAX_COUNT; species++)
        {
            ...
            var formList = FormConverter.GetFormList(species, strings.Types, strings.forms, GameInfo.GenderSymbolASCII, EntityContext.Gen9);
