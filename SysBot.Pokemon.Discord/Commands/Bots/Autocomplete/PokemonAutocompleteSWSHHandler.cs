...
public class PokemonAutocompleteSWSHHandler : AutocompleteHandler
{
    ...
    private static List<(string Display, string Value)> GetValidSpeciesForGame(GameVersion game)
    {
        ...
        for (ushort species = 1; species < (ushort)Species.MAX_COUNT; species++)
        {
            ...
            var formList = FormConverter.GetFormList(species, strings.Types, strings.forms, GameInfo.GenderSymbolASCII, EntityContext.Gen8);
    ...
    private static string BuildDisplayName(string baseName, string formName, ushort species, byte form)
    {
        ...
        // Special casing for Pikachu Cap forms: show "Pikachu-Alola", "Pikachu-Sinnoh", etc.
        if (species == (ushort)Species.Pikachu && formName.EndsWith(" Cap", StringComparison.OrdinalIgnoreCase))
        {
            var trimmed = formName[..^4].Trim(); // drop " Cap"
