...
namespace SysBot.Pokemon.Discord
{
    public class MysteryEggModule<T> : ModuleBase<SocketCommandContext> where T : PKM, new()
    {
        ...
        public async Task TradeMysteryEggAsync()
        {
            ...
            {
                await ReplyAsync(AppLocalization.Get(LocalizationKeys.DiscordMysteryEggUnavailable)).ConfigureAwait(false);
                return;
            }
            ...
            {
                await ReplyAsync(AppLocalization.Get(LocalizationKeys.DiscordAlreadyInQueue)).ConfigureAwait(false);
                return;
            }
        ...
        public async Task BatchMysteryEggAsync([Summary("Number of eggs (1-4)")] int count = 2)
        {
            ...
            if (count < 1 || count > maxEggs)
            {
                await Helpers<T>.ReplyAndDeleteAsync(Context,
                    AppLocalization.Format(LocalizationKeys.DiscordMysteryEggInvalidCount, maxEggs), 5);
                return;
            }

            var processingMessage = await Context.Channel.SendMessageAsync(AppLocalization.Format(LocalizationKeys.DiscordMysteryEggGenerating, Context.User.Mention, count));
            ...
            _ = Task.Run(async () =>
            {
                try
                {
                    ...
                    {
                        await Context.Channel.SendMessageAsync(AppLocalization.Format(LocalizationKeys.DiscordMysteryEggGenerateFailed, Context.User.Mention));
                        return;
                    }
                    ...
                    if (failedCount > 0)
                    {
                        await Context.Channel.SendMessageAsync(AppLocalization.Format(LocalizationKeys.DiscordMysteryEggGenerateWarning, Context.User.Mention, failedCount, batchEggList.Count));
                    }
                ...
                catch (Exception ex)
                {
                    ...
                    await Context.Channel.SendMessageAsync(AppLocalization.Format(LocalizationKeys.DiscordMysteryEggBatchError, Context.User.Mention));
        ...
        public async Task BatchTradeMysteryEggAsync([Summary("Number of Mystery Eggs")] int count)
        {
            ...
            {
                await Helpers<T>.ReplyAndDeleteAsync(Context,
                    AppLocalization.Format(LocalizationKeys.DiscordMysteryEggBatchLimit, maxTradesAllowed), 5);
                return;
            }
            ...
            _ = Task.Run(async () =>
            {
                try
                {
                    ...
                    if (failed > 0)
                    {
                        await Context.Channel.SendMessageAsync(AppLocalization.Format(LocalizationKeys.DiscordMysteryEggGenerateWarning, Context.User.Mention, failed, batchList.Count));
                    }
        ...
        private static async Task ProcessBatchMysteryEggs(SocketCommandContext context, List<T> batchEggList, int batchTradeCode, int totalEggs)
        {
            ...
            {
                await context.Channel.SendMessageAsync(AppLocalization.Get(LocalizationKeys.DiscordMysteryEggAlreadyInQueue)).ConfigureAwait(false);
                return;
            }
            ...
            await context.Channel.SendMessageAsync(AppLocalization.Format(LocalizationKeys.DiscordMysteryEggAddedBatch, context.User.Mention, batchEggList.Count, position.Position, baseEta)).ConfigureAwait(false);
        ...
        private static Embed CreateMysteryEggEmbed(SocketCommandContext context, T pk, int eggNumber, int totalEggs, int queuePosition)
        {
            var embedBuilder = new EmbedBuilder()
                ...
                .WithTitle($"🥚 {AppLocalization.Format(LocalizationKeys.DiscordMysteryEggTitle, eggNumber, totalEggs)}")
                .WithDescription(AppLocalization.Get(LocalizationKeys.DiscordMysteryEggDescription))
                .WithImageUrl(GetMysteryEggTypeImageUrl(pk))
                .WithFooter(AppLocalization.Format(LocalizationKeys.DiscordMysteryEggFooter, eggNumber, totalEggs) + (eggNumber == 1 ? $" | Position: {queuePosition}" : ""))
                .WithAuthor(new EmbedAuthorBuilder()
                    .WithName(AppLocalization.Format(LocalizationKeys.DiscordMysteryEggForUser, context.User.Username))
        ...
        private async Task ProcessMysteryEggTradeAsync(int code)
        {
            ...
            if (mysteryEgg == null)
            {
                await ReplyAsync(AppLocalization.Get(LocalizationKeys.DiscordMysteryEggLegalFailed)).ConfigureAwait(false);
                return;
            }
