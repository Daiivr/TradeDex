...
namespace SysBot.Pokemon.Discord
{
    public class MysteryMonModule<T> : ModuleBase<SocketCommandContext> where T : PKM, new()
    {
        ...
        public async Task MysteryMonAsync()
        {
            ...
            {
                await ReplyAsync(AppLocalization.Get(LocalizationKeys.DiscordAlreadyInQueue));
                return;
            }
        ...
        public async Task MysteryMonAsync(int code)
        {
            ...
            try
            {
                ...
                {
                    await ReplyAsync(AppLocalization.Get(LocalizationKeys.DiscordMysteryMonTryAgain));
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task BatchTradeMysteryMonAsync([Summary("Number of Mystery Pokémon")] int count)
        {
            ...
            if (!tradeConfig.AllowBatchTrades)
            {
                ...
                await Helpers<T>.ReplyAndDeleteAsync(Context,
                    AppLocalization.Format(LocalizationKeys.DiscordMysteryMonBatchDisabled, app.Owner), 6);
                return;
            }
            ...
            {
                await Helpers<T>.ReplyAndDeleteAsync(Context,
                    AppLocalization.Format(LocalizationKeys.DiscordMysteryMonBatchLimit, maxTradesAllowed), 5);
                return;
            }

            var processingMessage = await Context.Channel.SendMessageAsync(AppLocalization.Format(LocalizationKeys.DiscordMysteryMonGenerating, Context.User.Mention, count));
            ...
            _ = Task.Run(async () =>
            {
                try
                {
                    ...
                    {
                        await Context.Channel.SendMessageAsync(AppLocalization.Format(LocalizationKeys.DiscordMysteryMonGenerateFailed, Context.User.Mention));
                        return;
                    }

                    if (failed > 0)
                    {
                        await Context.Channel.SendMessageAsync(AppLocalization.Format(LocalizationKeys.DiscordMysteryMonGenerateWarning, Context.User.Mention, failed, batchList.Count));
                    }
                ...
                {
                    try { await processingMessage.DeleteAsync(); } catch { }
                    await Context.Channel.SendMessageAsync(AppLocalization.Format(LocalizationKeys.DiscordMysteryMonBatchError, Context.User.Mention));
                    LogUtil.LogError(AppLocalization.Format(LocalizationKeys.LogMysteryMonBatchError, ex.Message), nameof(BatchTradeMysteryMonAsync));
                }
            });
        ...
        private static T? GenerateMysteryMon(System.Threading.CancellationToken token)
        {
            ...
            for (int outer = 0; outer < 15; outer++)
            {
                ...
                LogUtil.LogInfo(AppLocalization.Format(LocalizationKeys.LogMysteryMonLegalityRetry, speciesName), "MysteryMon");
            }

            LogUtil.LogError(AppLocalization.Get(LocalizationKeys.LogMysteryMonAllSpeciesFailed), "MysteryMon");
            return default;
        }
        ...
        private static bool SafeApply(T pk, Action<T> action, string name, System.Threading.CancellationToken token, int maxAttempts, Func<bool>? preCheck = null)
        {
            ...
            for (int attempt = 0; attempt < maxAttempts; attempt++)
            {
                ...
                try
                {
                    ...
                    {
                        LogUtil.LogInfo(AppLocalization.Format(LocalizationKeys.LogMysteryMonStepOk, name, attempt + 1), "MysteryMon");
                        return true;
                    }
                    pk = backup; // restore
                }
            ...
            LogUtil.LogError(AppLocalization.Format(LocalizationKeys.LogMysteryMonStepFailed, name, maxAttempts), "MysteryMon");
            return false;
        }
        ...
        private static List<ushort> GetValidSpecies(GameVersion game)
        {
            ...
            if (table == null)
            {
                LogUtil.LogError(AppLocalization.Format(LocalizationKeys.LogMysteryMonNoPersonalTable, game), "MysteryMon");
