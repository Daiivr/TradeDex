...
#pragma warning restore CA1416 // Validate platform compatibility
        ...
        {
            var generatingMessage = await ReplyAsync(AppLocalization.Get(LocalizationKeys.DiscordPokepasteGenerating));
            try
            {
                await Task.Run(async () =>
                {
                    ...
                    var title = titleMatch.Success ? titleMatch.Groups[1].Value : AppLocalization.Get(LocalizationKeys.DiscordPokepasteDefaultTitle);
                    ...
                    {
                        await ReplyAndDeleteAsync(AppLocalization.Format(LocalizationKeys.DiscordPokepasteNoValidSets, pokePasteUrl), 10, generatingMessage).ConfigureAwait(false);
                        return;
                    }
                        ...
                        foreach (var set in showdownSets)
                        {
                            try
                            {
                                ...
                                using (var cts = new CancellationTokenSource(timeout))
                                {
                                    ...
                                    {
                                        await ReplyAndDeleteAsync(AppLocalization.Format(LocalizationKeys.DiscordPokepasteTimeout, GameInfo.Strings.Species[template.Species]), 10, generatingMessage).ConfigureAwait(false);
                                        continue;
                                    }
                                }
                                ...
                                {
                                    await ReplyAndDeleteAsync(AppLocalization.Format(LocalizationKeys.DiscordPokepasteCreateFailed, GameInfo.Strings.Species[template.Species]), 10, generatingMessage).ConfigureAwait(false);
                                    continue;
                                }
...
#pragma warning restore CA1416 // Validate platform compatibility
                            ...
                            {
                                var speciesName = GameInfo.GetStrings("en").Species[set.Species];
                                await ReplyAndDeleteAsync(AppLocalization.Format(LocalizationKeys.DiscordPokepasteProcessingError, speciesName, ex.Message), 10, generatingMessage).ConfigureAwait(false);
                            }
                        }
                    }
                    ...
                    await Context.User.SendFileAsync(memoryStream, $"{title}.zip", text: AppLocalization.Get(LocalizationKeys.DiscordPokepasteTeamDm)).ConfigureAwait(false);
...
#pragma warning restore CA1416 // Validate platform compatibility
                        ...
                        var embedBuilder = new EmbedBuilder()
                            ...
                            .WithAuthor(
                                ...
                                {
                                    author
                                        .WithName(AppLocalization.Format(LocalizationKeys.DiscordPokepasteGeneratedTeamAuthor, Context.User.Username))
                                        .WithIconUrl(Context.User.GetAvatarUrl() ?? Context.User.GetDefaultAvatarUrl());
                                })
                            .WithImageUrl($"attachment://{title}.png")
                            .WithFooter(AppLocalization.Format(LocalizationKeys.DiscordPokepasteTeamSentFooter, Context.User.Username))
            ...
            {
                await ReplyAndDeleteAsync(AppLocalization.Format(LocalizationKeys.DiscordPokepasteFatalError, ex.Message), 10, generatingMessage).ConfigureAwait(false);
            }
        }
