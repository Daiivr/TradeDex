...
public class SeedCheckModule<T> : ModuleBase<SocketCommandContext> where T : PKM, new()
{
    ...
    public async Task FindFrameAsync([Remainder] string seedString)
    {
        ...
        embed.AddField(x =>
        {
            x.Name = AppLocalization.Format(LocalizationKeys.DiscordSeedFieldName, seed);
            x.Value = msg;
            x.IsInline = false;
        });
        await ReplyAsync(AppLocalization.Format(LocalizationKeys.DiscordSeedDetails, r.Seed), embed: embed.Build()).ConfigureAwait(false);
    }
