...
public static class CreatePokemonHelper
{
        ...
        Action<T>? postProcessing = null) where T : PKM, new()
    {
        ...
        {
            await context.Interaction.FollowupAsync(AppLocalization.Get(LocalizationKeys.DiscordSlashPokemonEmpty), ephemeral: true).ConfigureAwait(false);
            return false;
        }
        ...
        {
            await context.Interaction.FollowupAsync(AppLocalization.Format(LocalizationKeys.DiscordSlashPokemonInvalid, speciesInput), ephemeral: true).ConfigureAwait(false);
            return false;
        }
        ...
        {
            var errorMsg = processed.Error is { Length: > 0 } error
                ? AppLocalization.Format(LocalizationKeys.DiscordGenericError, error)
                : AppLocalization.Get(LocalizationKeys.DiscordSlashGenerationUnknownError);
        ...
        if (!DetailedLegalityChecker.IsLegalWithDetailedReport(pk, displayName, commandPrefix, out string? legalityError))
        {
            ...
            await context.Interaction.FollowupAsync(AppLocalization.Format(LocalizationKeys.DiscordSlashIllegalPokemon, legalityError ?? AppLocalization.Get(LocalizationKeys.BotStatusUnknown)), ephemeral: true).ConfigureAwait(false);
            return false;
        }
        ...
        {
            await context.Interaction.FollowupAsync(AppLocalization.Get(LocalizationKeys.DiscordSlashAlreadyInQueue), ephemeral: true).ConfigureAwait(false);
            return false;
        }

        if (added == QueueResultAdd.QueueFull)
        {
            await context.Interaction.FollowupAsync(AppLocalization.Get(LocalizationKeys.DiscordSlashQueueFull), ephemeral: true).ConfigureAwait(false);
            return false;
        }
        ...
        if (embedData.IsLocalFile)
        {
            ...
            await context.Interaction.FollowupAsync(AppLocalization.Get(LocalizationKeys.DiscordSlashAddedQueueDm), ephemeral: true).ConfigureAwait(false);
