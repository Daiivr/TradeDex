...
public class LogModule : ModuleBase<SocketCommandContext>
{
    ...
    public async Task AddLogAsync()
    {
        ...
        await ReplyAsync(AppLocalization.Get(LocalizationKeys.DiscordLogAddedHere)).ConfigureAwait(false);
    }
    ...
    public async Task ClearLogsAllAsync()
    {
        ...
        {
            var entry = l.Value;
            await ReplyAsync(AppLocalization.Format(LocalizationKeys.DiscordLogClearedFrom, entry.ChannelName, entry.ChannelID)).ConfigureAwait(false);
        ...
        await ReplyAsync(AppLocalization.Get(LocalizationKeys.DiscordLogClearedAll)).ConfigureAwait(false);
    }
    ...
    public async Task ClearLogsAsync()
    {
        ...
        {
            await ReplyAsync(AppLocalization.Get(LocalizationKeys.DiscordLogNotInChannel)).ConfigureAwait(false);
            return;
        }
        LogUtil.Forwarders.Remove(log);
        Channels.Remove(Context.Channel.Id);
        SysCordSettings.Settings.LoggingChannels.RemoveAll(z => z.ID == id);
        await ReplyAsync(AppLocalization.Format(LocalizationKeys.DiscordLogClearedChannel, Context.Channel.Name)).ConfigureAwait(false);
    }
