...
public sealed class RequireQueueRoleAttribute(string RoleName) : PreconditionAttribute
{
    ...
    public override Task<PreconditionResult> CheckPermissionsAsync(ICommandContext context, CommandInfo command, IServiceProvider services)
    {
        ...
        if (!canQueue)
            return Task.FromResult(PreconditionResult.FromError(AppLocalization.Format(LocalizationKeys.DiscordPreconditionQueueClosed, context.User.Mention)));

        if (!mgr.GetHasRoleAccess(RoleName, roles.Select(z => z.Name)))
            return Task.FromResult(PreconditionResult.FromError(AppLocalization.Format(LocalizationKeys.DiscordPreconditionRoleRequired, context.User.Mention)));
