...
public class TwitchBot<T> : IChatBot where T : PKM, new()
{
    ...
    public void SendMessage(string message)
    {
        try
        {
            ...
            {
                client.SendMessage(Settings.Channel, Localize(message));
            }
        }
        catch (Exception ex)
        {
    ...
    private void SendWhisper(string user, string message)
    {
        try
        {
            if (isConnected && client?.IsConnected == true)
            {
                client.SendWhisper(user, Localize(message));
            }
        }
        catch (Exception ex)
        {
    ...
    private bool AddToTradeQueue(T pk, int code, OnWhisperReceivedArgs e, RequestSignificance sig, PokeRoutineType type, out string msg)
    {
        ...
        if (client == null)
        {
            msg = Localize($"@{name}: Twitch client is not connected. Please try again later.");
            return false;
        }
        ...
        if (TradeExtensions<T>.IsItemBlocked(pk))
        {
            ...
            msg = Localize($"@{name}: Trade blocked — the held item '{itemName}' cannot be traded.");
            return false;
        }
        ...
        {
            msg = Localize($"@{name}: Sorry, you are already in the queue.");
            return false;
        }
        ...
        if (added == QueueResultAdd.NotAllowedItem)
        {
            ...
            msg = Localize($"@{name}: Trade blocked — the held item '{itemName}' cannot be traded in PLZA.");
            return false;
        }

        var position = Info.CheckPosition(userID, uniqueTradeID, type);
        msg = Localize($"@{name}: Added to the {type} queue, unique ID: {detail.ID}. Current Position: {position.Position}");

        var botct = Info.Hub.Bots.Count;
        if (position.Position > botct)
        {
            var eta = Info.Hub.Config.Queues.EstimateDelay(position.Position, botct);
            msg += Localize($". Estimated: {eta:F1} minutes.");
        }
        return true;
    }
    ...
    private string HandleCommand(TwitchLibMessage m, string c, string args, bool whisper)
    {
        ...
        {
            case "donate":
                return Settings.DonationLink.Length > 0 ? Localize($"Here's the donation link! Thank you for your support :3 {Settings.DonationLink}") : string.Empty;

            case "discord":
                return Settings.DiscordLink.Length > 0 ? Localize($"Here's the Discord Server Link, have a nice stay :3 {Settings.DiscordLink}") : string.Empty;
            ...
            case "position":
                ...
                if (tradeEntry != null)
                {
                    ...
                    return Localize($"@{m.Username}: {Info.GetPositionString(userID, uniqueTradeID)}");
                }
                else
                {
                    return Localize($"@{m.Username}: You are not currently in the queue.");
                }
            case "tc":
            case "cancel":
            case "remove":
                return Localize($"@{m.Username}: {TwitchCommandsHelper<T>.ClearTrade(ulong.Parse(m.UserId))}");

            case "code" when whisper:
                return Localize(TwitchCommandsHelper<T>.GetCode(ulong.Parse(m.UserId)));
            ...
            case "tt" when !sudo():
            case "tcu" when !sudo():
                return Localize("This command is locked for sudo users only!");

            case "tca":
                Info.ClearAllQueues();
                return Localize("Cleared all queues!");

            case "pr":
                return Localize(Info.Hub.Ledy.Pool.Reload(Hub.Config.Folder.DistributeFolder) ? $"Reloaded from folder. Pool count: {Info.Hub.Ledy.Pool.Count}" : "Failed to reload from folder.");

            case "pc":
                return Localize($"The pool count is: {Info.Hub.Ledy.Pool.Count}");

            case "tt":
                return Info.Hub.Queues.Info.ToggleQueue()
                    ? Localize("Users are now able to join the trade queue.")
                    : Localize("Changed queue settings: **Users CANNOT join the queue until it is turned back on.**");

            case "tcu":
                return Localize(TwitchCommandsHelper<T>.ClearTrade(args));
