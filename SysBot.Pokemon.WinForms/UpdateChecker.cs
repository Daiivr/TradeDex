...
namespace SysBot.Pokemon.WinForms
{
    public class UpdateChecker
    {
        ...
        private const string RepositoryName = "TradeDex";
        ...
        {
            _httpClient.DefaultRequestHeaders.Add("User-Agent", "TradeDex");
        }
        ...
        {
            ReleaseInfo? latestRelease = await FetchLatestReleaseAsync();
            return latestRelease?.Body ?? AppLocalization.LocalizeRuntimeMessage("Failed to fetch the latest release information.");
        }
        ...
        private static async Task<ReleaseInfo?> FetchLatestReleaseAsync()
        {
            try
            {
                ...
                if (!response.IsSuccessStatusCode)
                {
                    ...
                    Console.WriteLine(AppLocalization.LocalizeRuntimeMessage($"GitHub API Error: {response.StatusCode} - {errorContent}"));

                    // Provide more helpful error messages
                    if (response.StatusCode == System.Net.HttpStatusCode.Forbidden)
                    {
                        Console.WriteLine(AppLocalization.LocalizeRuntimeMessage("GitHub API rate limit may have been exceeded. Try again later."));
                    }
                    else if (response.StatusCode == System.Net.HttpStatusCode.NotFound)
                    {
                        Console.WriteLine(AppLocalization.LocalizeRuntimeMessage($"Repository {RepositoryOwner}/{RepositoryName} or latest release not found."));
                    }
                ...
                if (releaseInfo != null)
                {
                    Console.WriteLine(AppLocalization.LocalizeRuntimeMessage($"Successfully fetched release info: Version {releaseInfo.TagName}"));
                }
            ...
            {
                Console.WriteLine(AppLocalization.LocalizeRuntimeMessage($"Network error fetching release info: {ex.Message}"));
                Console.WriteLine(AppLocalization.LocalizeRuntimeMessage("Please check your internet connection and try again."));
                return null;
            }
            catch (TaskCanceledException ex)
            {
                Console.WriteLine(AppLocalization.LocalizeRuntimeMessage($"Request timeout fetching release info: {ex.Message}"));
                Console.WriteLine(AppLocalization.LocalizeRuntimeMessage("The request took too long. Please try again."));
                return null;
            }
            catch (Exception ex)
            {
                Console.WriteLine(AppLocalization.LocalizeRuntimeMessage($"Unexpected error fetching release info: {ex.Message}"));
                return null;
            }
        }
