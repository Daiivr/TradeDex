...
public partial class BotServer(Main mainForm, int port = 8080, int tcpPort = 8081) : IDisposable
{
    ...
    private async Task<(int statusCode, object? content, string contentType)> ProcessRequestAsync(HttpListenerRequest request, HttpListenerResponse response)
    {
        ...
        return path switch
        {
            ...
            "/bdsp_mode_image.png" => (200, LoadEmbeddedResourceBinary("bdsp_mode_image.png"), "image/png"),
            "/pla_mode_image.png" => (200, LoadEmbeddedResourceBinary("pla_mode_image.png"), "image/png"),
            "/lgpe_mode_image.png" => (200, LoadEmbeddedResourceBinary("lgpe_mode_image.png"), "image/png"),
    ...
    private async Task<string> UpdateAllInstances(HttpListenerRequest request)
    {
        try
        {
            ...
            return JsonSerializer.Serialize(new
            {
                ...
                success = true,
                info = L("Update process started in background. Use /api/bot/update/active to check status.")
            }, JsonOptions);
        }
        catch (Exception ex)
        {
        ...
        try
        {
            ...
            if (port == _tcpPort)
            {
                ...
                try
                {
                    ...
                    {
                        success = true,
                        message = L("Master instance update initiated - updating all instances"),
                        sessionId = session.SessionId,
                        redirectedToUpdateAll = true,
                        port,
                        info = L("Use /api/bot/update/active to check status")
                    }, JsonOptions);
                }
                catch (Exception ex)
                {
            ...
            if (UpdateManager.IsUpdateInProgress())
            {
                ...
                if (currentState?.Instances?.Any(i => i.TcpPort == port) == true)
                {
                    ...
                    {
                        success = false,
                        message = L($"Instance {port} is already being updated"),
            ...
            if (success)
            {
                ...
                {
                    success = true,
                    message = L($"Instance on port {port} updated successfully"),
                    port,
            ...
            else
            {
                ...
                {
                    success = false,
                    message = L($"Failed to update instance on port {port}"),
                    port,
                    error = L("Update failed - check logs for details")
                }, JsonOptions);
            }
        }
    ...
    private BotInstance CreateLocalInstance()
    {
        ...
        var name = config?.Hub?.BotName ?? "TradeDex";
    ...
    private async Task<List<BotInstance>> ScanRemoteInstancesAsync()
    {
        ...
        try
        {
            var processes = Process.GetProcessesByName("TradeDex")
            ...
            foreach (var process in processes)
            {
                try
                {
                    ...
                    var botName = "TradeDex"; // default
                    if (!string.IsNullOrEmpty(exePath))
                    {
                        botName = Path.GetFileNameWithoutExtension(exePath);
                    }

                    var portFile = Path.Combine(Path.GetDirectoryName(exePath)!, $"TradeDex_{process.Id}.port");
        ...
        try
        {
            ...
            foreach (var proc in processes)
            {
                try
                {
                    ...
                    var portFile = Path.Combine(Path.GetDirectoryName(exePath)!, $"TradeDex_{proc.Id}.port");
                    if (File.Exists(portFile))
                    {
                        var portText = File.ReadAllText(portFile).Trim();
        ...
        try
        {
            ...
            if (infoResponse.StartsWith('{'))
            {
                ...
                if (root.TryGetProperty("Name", out var name))
                    instance.Name = name.GetString() ?? "TradeDex";
    ...
    public static string QueryRemote(int port, string command)
    {
        try
        {
            ...
            {
                return L("ERROR: Invalid command");
            }
            ...
            {
                return L("ERROR: Command contains invalid characters");
            }
            ...
            {
                return L("ERROR: Connection timeout");
            }
            ...
            return response ?? L("ERROR: No response");
        }
        catch (Exception ex)
        {
    ...
    private string GetQueueStatus()
    {
        try
        {
            ...
            if (config?.Hub == null)
            {
                return JsonSerializer.Serialize(new
                {
                    ...
                    isFull = false,
                    canQueue = true,
                    message = L("Queue information unavailable")
                }, JsonOptions);
            }
            ...
            if (hubProperty?.GetValue(config.Hub) is not object queues)
            {
                return JsonSerializer.Serialize(new
                {
                    ...
                    message = L("Queue system not initialized")
                }, JsonOptions);
            }
            ...
            if (infoProperty?.GetValue(queues) is not object info)
            {
                return JsonSerializer.Serialize(new
                {
                    ...
                    isFull = false,
                    canQueue = config.Hub.Queues.CanQueue,
                    message = L("Queue info not available")
                }, JsonOptions);
            }
        ...
        catch (Exception ex)
        {
            ...
            return JsonSerializer.Serialize(new
            {
                ...
                isFull = false,
                canQueue = false,
                message = L("Error retrieving queue status")
            }, JsonOptions);
        }
    }
    ...
    {
        return JsonSerializer.Serialize(ApiResponseFactory.CreateSimpleError(L(message)), JsonOptions);
    }
    ...
    private static string ClearUpdateSession()
    {
        try
        {
            ...
            if (currentState != null)
            {
                ...
                if (currentVersion == currentState.TargetVersion)
                {
                    if (!currentState.IsComplete || !currentState.Success)
                    {
                        ...
                        {
                            success = true,
                            message = L("Update completed successfully - all instances updated to target version"),
                    ...
                    else
                    {
                        ...
                        {
                            success = true,
                            message = L("Update was already successful - session cleared"),
                            action = "cleared",
                            currentVersion
                        }, JsonOptions);
                    }
            ...
            {
                success = true,
                message = L("Update session cleared"),
                action = "cleared"
            }, JsonOptions);
        }
        catch (Exception ex)
    ...
    private async Task<string> ExecuteLocalRemoteButton(string button, int botIndex = 0)
    {
        try
        {
            ...
            {
                success = true,
                message = L($"Button {button} pressed on bot {botIndex}"),
                button,
                botIndex
            }, JsonOptions);
        }
    ...
    private async Task<string> ExecuteLocalRemoteMacro(string macro, int botIndex = 0)
    {
        try
        {
            ...
            {
                success = true,
                message = L($"Macro executed successfully on bot {botIndex}"),
