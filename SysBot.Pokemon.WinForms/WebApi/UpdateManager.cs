...
public static class UpdateManager
{
    ...
    private static async Task<List<Instance>> DiscoverInstancesSimpleAsync(int currentTcpPort)
    {
        ...
        try
        {
            var otherProcesses = Process.GetProcessesByName("TradeDex")
    ...
    private static int GetCurrentPort()
    {
        ...
        {
            var baseDir = Path.GetDirectoryName(Application.ExecutablePath) ?? "";
            var portFile = Path.Combine(baseDir, $"TradeDex_{Environment.ProcessId}.port");
    ...
    {
        // Extract original filename from URL (e.g., "  usionBot.exe" from GitHub release)
    ...
    private static void InstallUpdateAndRestart(string downloadedFilePath)
    {
        try
        {
            ...
            string targetExeName = "TradeDex.exe";
