...
public static class WebApiExtensions
{
    ...
    private static void CleanupStalePortFiles()
    {
        try
        {
            ...
            var portFiles = Directory.GetFiles(exeDir, "TradeDex_*.port");
            ...
            foreach (var portFile in portFiles)
            {
                try
                {
                    ...
                    var pidStr = fileName.Substring("TradeDex_".Length);
                    ...
                    if (int.TryParse(pidStr, out int pid))
                    {
                        ...
                        try
                        {
                            ...
                            if (process.ProcessName.Contains("SysBot", StringComparison.OrdinalIgnoreCase) ||
                                process.ProcessName.Contains("TradeDex", StringComparison.OrdinalIgnoreCase))
                            {
                                continue;
                            }
                        }
    ...
    {
        if (_main == null)
            return L("ERROR: Main form not initialized");
        ...
        try
        {
            lock (_updateLock)
            {
                if (_updateInProgress)
                {
                    ...
                    return L("Update already in progress");
                }
            ...
            return L("OK: Update triggered");
        }
        catch (Exception ex)
        {
            lock (_updateLock) { _updateInProgress = false; }
            return L($"ERROR: {ex.Message}");
        }
    }
    ...
    private static string ExecuteGlobalCommand(BotControlCommand command)
    {
        try
        {
            ...
            return L($"OK: {command} command sent to all bots");
        }
        catch (Exception ex)
        {
            return L($"ERROR: Failed to execute {command} - {ex.Message}");
        }
    }
    ...
    private static string GetBotsList()
    {
        ...
        catch (Exception ex)
        {
            ...
            return L($"ERROR: Failed to get bots list - {ex.Message}");
        }
    }
    ...
    private static string GetBotStatuses(string? botId)
    {
        try
        {
            ...
            return botController?.ReadBotState() ?? L("ERROR: Bot not found");
        }
        catch (Exception ex)
        {
            return L($"ERROR: Failed to get status - {ex.Message}");
        }
    }
    ...
    private static string GetInstanceInfo()
    {
        ...
        {
            return L($"ERROR: Failed to get instance info - {ex.Message}");
        }
    }
    ...
    private static string HandleRemoteButton(string[] parts)
    {
        ...
        {
            if (parts.Length < 3)
                return L("ERROR: Invalid command format. Expected REMOTE_BUTTON:button:botIndex");

            var button = parts[1];
            if (!int.TryParse(parts[2], out var botIndex))
                return L("ERROR: Invalid bot index");

            var controllers = GetBotControllers();
            if (botIndex < 0 || botIndex >= controllers.Count)
                return L($"ERROR: Bot index {botIndex} out of range");

            var botController = controllers[botIndex];
            var botSource = botController.GetBot();

            if (botSource?.Bot == null)
                return L($"ERROR: Bot at index {botIndex} not available");

            if (!botSource.IsRunning)
                return L($"ERROR: Bot at index {botIndex} is not running");

            var bot = botSource.Bot;
            if (bot.Connection is not ISwitchConnectionAsync connection)
                return L("ERROR: Bot connection not available");

            var switchButton = MapButtonToSwitch(button);
            if (switchButton == null)
                return L($"ERROR: Invalid button: {button}");
            ...
            return L($"OK: Button {button} pressed on bot {botIndex}");
        }
        catch (Exception ex)
        {
            return L($"ERROR: {ex.Message}");
    ...
    private static string HandleRemoteMacro(string[] parts)
    {
        ...
        {
            if (parts.Length < 3)
                return L("ERROR: Invalid command format. Expected REMOTE_MACRO:macro:botIndex");
            ...
            return L($"OK: Macro {macro} executed on bot {botIndex}");
        }
        catch (Exception ex)
        {
            return L($"ERROR: {ex.Message}");
    ...
    private static void CreatePortFile()
    {
        try
        {
            var exePath = System.Diagnostics.Process.GetCurrentProcess().MainModule?.FileName ?? "";
            var exeDir = Path.GetDirectoryName(exePath) ?? Program.WorkingDirectory;
            var portFile = Path.Combine(exeDir, $"TradeDex_{Environment.ProcessId}.port");
