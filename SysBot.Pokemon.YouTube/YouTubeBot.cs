...
public class YouTubeBot<T> : IDisposable where T : PKM, new()
{
    ...
    public YouTubeBot(YouTubeSettings settings, PokeTradeHub<T> hub)
    {
        ...
        Task.Run(async () =>
        {
            try
            {
                ...
                _echoForwarder = msg => client.SendMessage(Localize(msg));
    ...
    public void StartingDistribution(string message)
    {
        Task.Run(async () =>
        {
            ...
            if (!string.IsNullOrWhiteSpace(message))
                await client.SendMessage(Localize(message)).ConfigureAwait(false);
        });
    }
        ...
        foreach (var message in messages)
        {
            ...
            try
            {
                ...
                if (response.Length == 0)
                    return;
                client?.SendMessage(Localize(response));
            }
            catch
            {
                // ignored
    ...
    private string HandleCommand(LiveChatMessage m, string cmd, string args)
    {
        ...
        if (args.Length > 0)
            return Localize("Commands don't use arguments. Try again with just the command code.");

        return cmd switch
        {
            "pr" => (Info.Hub.Ledy.Pool.Reload(Hub.Config.Folder.DistributeFolder)
                ? Localize($"Reloaded from folder. Pool count: {Info.Hub.Ledy.Pool.Count}")
                : Localize("Failed to reload from folder.")),

            "pc" => Localize($"The pool count is: {Info.Hub.Ledy.Pool.Count}"),
