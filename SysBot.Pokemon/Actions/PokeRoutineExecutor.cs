    ...
    where T : PKM, new()
{
    ...
    public static void DumpPokemon(string folder, string subfolder, T pk)
    {
        ...
        LogUtil.LogInfo("Volcado", $"Archivo guardado: {fn}");
    }
    ...
    public async Task CheckForRAMShiftingApps(CancellationToken token)
    {
        ...
        if (found)
        {
            msg += ".";
            Log(msg);
            Log("Por favor, elimina las aplicaciones que interfieren y reinicia la Switch.");
        }
    }
        ...
        TradeAbuseSettings AbuseSettings, CancellationToken token)
    {
        ...
        if (previous != null)
        {
            ...
            {
                Log($"Se encontró a {user.TrainerName} ignorando el enfriamiento de intercambio de {cd} minutos. Último encuentro hace {delta.TotalMinutes:F1} minutos.");
                return PokeTradeResult.SuspiciousActivity;
            }
            ...
            if (isDistribution && previous.NetworkID == TrainerNID && previous.RemoteID != user.ID)
            {
                ...
                {
                    quit = true;
                    Log($"Se encontró a {user.TrainerName} usando múltiples cuentas.\nAnteriormente intercambió con {previous.Name} ({previous.RemoteID}) hace {delta.TotalMinutes:F1} minutos en OT: {TrainerName}.");
                }
            }
        }
