...
public abstract class PokeRoutineExecutor8BS : PokeRoutineExecutor<PB8>
{
    ...
    public async Task<SAV8BS> IdentifyTrainer(CancellationToken token)
    {
        ...
        Offsets = title switch
        {
            ...
            _ => throw new Exception($"{title} no es un título válido de Pokémon BDSP. ¿Tu modo es correcto?"),
        };

        // Verify the game version.
        var game_version = await SwitchConnection.GetGameInfo("version", token).ConfigureAwait(false);
        if (!game_version.SequenceEqual(BSGameVersion))
            throw new Exception($"La versión del juego no es compatible. Se esperaba la versión {BSGameVersion}, y la versión actual del juego es {game_version}.");
        ...
        if (await GetTextSpeed(token).ConfigureAwait(false) < TextSpeedOption.Fast)
            throw new Exception("La velocidad de texto debe estar configurada en RÁPIDO. Corrige esto para un funcionamiento adecuado.");
    ...
    public async Task InitializeHardware(IBotStateSettings settings, CancellationToken token)
    {
        ...
        Log("Configurando esperas HID específicas de BDSP.");
            ...
            if (totalWaitTime >= maxWaitTime)
            {
                if (!timing.AvoidSystemUpdate)
                {
                    ...
                    while (!await IsSceneID(SceneID_Field, token).ConfigureAwait(false))
                    {
                        ...
                        {
                            Log("Se alcanzó el número máximo de reintentos al intentar iniciar el juego.");
                            throw new Exception("No se pudo iniciar el juego después de múltiples intentos.");
                        }
    ...
    {
        Log("Se detectó un soft ban, desbaneando.");
