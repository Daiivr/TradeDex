    ...
    private static readonly ushort[] ExplicitlyBlockedHeldItems =
{
    ...
    public static readonly string[] MarkTitleEnglish =
    [
        "the Peckish",
            "the Sleepy",
            "the Dozy",
    ...
    public static T DittoTrade(PKM pkm, int[]? preservedIVs = null)
    {
        ...
        pkm.Ball = 21;
        pkm.IVs = preservedIVs is { Length: 6 }
            ? [.. preservedIVs]
    ...
    public static bool HasMark(IRibbonIndex pk, out RibbonIndex result, out string markTitle)
    {
        ...
        if (pk is IRibbonSetMark9 ribbonSetMark)
        {
            if (ribbonSetMark.RibbonMarkMightiest)
            {
                ...
                markTitle = GetLocalizedMarkTitle("the Unrivaled", "el Imbatible");
                return true;
            }
            else if (ribbonSetMark.RibbonMarkAlpha)
            {
                ...
                if (pk is PA9 pa9)
                {
                    ...
                    markTitle = isNative
                        ? GetLocalizedMarkTitle("the Alpha", "el Alfa")
                        : GetLocalizedMarkTitle("the Former Alpha", "el Antiguo Alfa");
                }
                else
                {
                    markTitle = GetLocalizedMarkTitle("the Former Alpha", "el Antiguo Alfa");
                }
                return true;
            }
            else if (ribbonSetMark.RibbonMarkTitan)
            {
                ...
                markTitle = GetLocalizedMarkTitle("the Former Titan", "el Antiguo Dominante");
                return true;
            }
            else if (ribbonSetMark.RibbonMarkJumbo)
            {
                result = RibbonIndex.MarkJumbo;
                markTitle = GetLocalizedMarkTitle("the Great", "el Gigante");
                return true;
            }
            else if (ribbonSetMark.RibbonMarkMini)
            {
                result = RibbonIndex.MarkMini;
                markTitle = GetLocalizedMarkTitle("the Teeny", "el Diminuto");
                return true;
            }
        }
        ...
        for (var mark = RibbonIndex.MarkLunchtime; mark <= RibbonIndex.MarkSlump; mark++)
        {
            ...
            {
                result = mark;
                markTitle = GetLocalizedMarkTitle((int)mark - (int)RibbonIndex.MarkLunchtime);
                return true;
            }
        }
    ...
    private static string GetLocalizedMarkTitle(int index) =>
        AppLocalization.Language == AppLanguage.Spanish
            ? MarkTitle[index]
            : MarkTitleEnglish[index];

    private static string GetLocalizedMarkTitle(string english, string spanish) =>
    ...
    public static string PokeImg(PKM pkm, bool canGmax, bool fullSize, ImageSize? preferredImageSize = null)
    {
        ...
        if (fullSize)
        {
            baseLink = "https://raw.githubusercontent.com/Secludedly/ZE-FusionBot-HOME-Images/master/512x512/poke_capture_0001_000_mf_n_00000000_f_n.png".Split('_');
        }
        else if (preferredImageSize.HasValue)
        {
            baseLink = preferredImageSize.Value switch
            {
                ImageSize.Size256x256 => "https://raw.githubusercontent.com/Secludedly/ZE-FusionBot-HOME-Images/master/256x256/poke_capture_0001_000_mf_n_00000000_f_n.png".Split('_'),
                ImageSize.Size128x128 => "https://raw.githubusercontent.com/Secludedly/ZE-FusionBot-HOME-Images/master/128x128/poke_capture_0001_000_mf_n_00000000_f_n.png".Split('_'),
                _ => "https://raw.githubusercontent.com/Secludedly/ZE-FusionBot-HOME-Images/master/256x256/poke_capture_0001_000_mf_n_00000000_f_n.png".Split('_'),
            };
        }
        else
        {
            baseLink = "https://raw.githubusercontent.com/Secludedly/ZE-FusionBot-HOME-Images/master/256x256/poke_capture_0001_000_mf_n_00000000_f_n.png".Split('_');
        }
        ...
        if (pkm.Species is (ushort)Species.Basculegion)
        {
            ...
            return "https://raw.githubusercontent.com/Secludedly/ZE-FusionBot-HOME-Images/master/256x256/poke_capture_0" + $"{pkm.Species}" + "_00" + $"{pkm.Form}" + "_" + $"{g}" + "_n_00000000_f_" + $"{s}" + ".png";
        }
    ...
    public static bool IsItemBlocked(PKM pkm)
    {
        ...
        if (ExplicitlyBlockedHeldItems.Contains((ushort)held))
        {
            Base.LogUtil.LogInfo($"Intento de intercambio de Orbe Primigenio bloqueado (ID del objeto: {held})", "BlockItem");
            return true;
        }
