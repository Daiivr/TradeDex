...
public class PokeTradeBotLA(PokeTradeHub<PA8> Hub, PokeBotState Config) : PokeRoutineExecutor8LA(Config), ICountBot, ITradeBot
{
    ...
    public override async Task MainLoop(CancellationToken token)
    {
        try
        {
            ...
            OnConnectionSuccess();
            Log($"Iniciando el bucle principal de {nameof(PokeTradeBotLA)}.");
        ...
        Log($"Finalizando el bucle de {nameof(PokeTradeBotLA)}.");
    ...
    private async Task DoNothing(CancellationToken token)
    {
        ...
        {
            if (waitCounter == 0)
                Log("No hay ninguna tarea asignada. Esperando una nueva asignación de tarea.");
    ...
    private async Task<PokeTradeResult> PerformBatchTrade(SAV8LA sav, PokeTradeDetail<PA8> poke, CancellationToken token)
    {
        ...
        void SendCollectedPokemonAndCleanup()
        {
            ...
            if (allReceived.Count > 0)
            {
                ...
                for (int j = 0; j < allReceived.Count; j++)
                {
                    ...
                    Log($"  - Devolviendo: {GameInfo.GetStrings("en").Species[pokemon.Species]} (Checksum: {pokemon.Checksum:X8})");
        ...
        for (int i = 0; i < totalBatchTrades; i++)
        {
            ...
            if (currentTradeIndex == 0)
            {
                ...
                if (token.IsCancellationRequested)
                {
                    if (startingDetail.TotalBatchTrades > 1)
                        poke.SendNotification(this, "⚠️ Cancelando los intercambios por lotes. La rutina ha sido interrumpida.");
                    TradeProgressChanged?.Invoke(0);
                ...
                {
                    if (startingDetail.TotalBatchTrades > 1)
                        poke.SendNotification(this, "⚠️ No se encontró ningún jugador. Cancelando los intercambios por lotes.");
                    else
                        poke.SendNotification(this, "⚠️ No se encontró ningún jugador. Cancelando el intercambio.");
                    TradeProgressChanged?.Invoke(0);
                ...
                {
                    if (startingDetail.TotalBatchTrades > 1)
                        poke.SendNotification(this, "⚠️ La verificación del compañero falló. Cancelando los intercambios por lotes.");
                    TradeProgressChanged?.Invoke(0);
            ...
            {
                if (startingDetail.TotalBatchTrades > 1)
                    poke.SendNotification(this, $"⚠️ El compañero de intercambio tardó demasiado en el intercambio {currentTradeIndex + 1}/{totalBatchTrades}. Cancelando los intercambios restantes.");
                TradeProgressChanged?.Invoke(0);
            ...
            if (offered == null)
            {
                Log("No se pudo leer el Pokémon ofrecido, pero continuando con el intercambio de todas formas.");
                offered = new PA8();
            }
            ...
            {
                if (startingDetail.TotalBatchTrades > 1)
                    poke.SendNotification(this, "⚠️ Cancelando los intercambios por lotes restantes. La rutina ha sido interrumpida.");
                TradeProgressChanged?.Invoke(0);
            ...
            LogSuccessfulTrades(poke, cachedTradePartner?.NID ?? 0, cachedTradePartner?.TrainerName ?? "Desconocido");
...
#pragma warning restore CS8604 // Possible null reference argument.
        ...
        */
        ...
        {
            Log("No se pudo leer el Pokémon ofrecido, pero continuando con el intercambio de todas formas.");
            offered = new PA8(); // Crear PA8 vacío para evitar excepciones de referencia nula
        }
        ...
        Log("Confirmando el intercambio.");
        TradeProgressChanged?.Invoke(90);
    ...
    {
        if (unexpected)
            Log("Comportamiento inesperado, recuperando la posición.");
    ...
    private async Task<PokeTradeResult> ProcessDumpTradeAsync(PokeTradeDetail<PA8> detail, CancellationToken token)
    {
        ...
        Log($"Se terminó el ciclo de Dump después de procesar {ctr} Pokémon.");
        TradeProgressChanged?.Invoke(100);
        ...
        detail.Notifier.SendNotification(this, detail, $"Se descargaron {ctr} Pokémon.");
    ...
    private async Task<(PA8 toSend, PokeTradeResult check)> HandleClone(SAV8LA sav, PokeTradeDetail<PA8> poke, PA8 offered, CancellationToken token)
    {
        ...
        if (!await CheckCloneChangedOffer(token).ConfigureAwait(false))
        {
            ...
            poke.SendNotification(this, "**¡¡OYE, CÁMBIALO AHORA O ME VOY!!**");
            if (!await CheckCloneChangedOffer(token).ConfigureAwait(false))
            {
    ...
    private async Task<(PA8 toSend, PokeTradeResult check)> HandleFixOT(SAV8LA sav, PokeTradeDetail<PA8> poke, PA8 offered, PartnerDataHolder partner, CancellationToken token)
    {
        ...
        {
            poke.SendNotification(this, "✅ No se detectó publicidad en el Apodo o el OT, y el Pokémon es legal. Saliendo del intercambio.");
            TradeProgressChanged?.Invoke(0);
        ...
        if (!laInit.Valid)
        {
            ...
            Log(laInit.Report());
            poke.SendNotification(this, $"**El Pokémon mostrado no es legal. Intentando regenerarlo...**\n\n```{report}```");
            if (DumpSetting.Dump)
                DumpPokemon(DumpSetting.DumpFolder, "hacked", offered);
        }
        ...
        if (!la.Valid)
        {
            poke.SendNotification(this, "✅ Este Pokémon no es legal según las verificaciones de legalidad de PKHeX. No pude corregirlo. Saliendo del intercambio.");
            return (clone, PokeTradeResult.IllegalTrade);
        }

        TradeExtensions<PA8>.HasAdName(offered, out string detectedAd);
        poke.SendNotification(this, $"{(!laInit.Valid ? "**Legalizado" : "**Arreglado Apodo/OT para")} {(Species)clone.Species}** (publicidad detectada: {detectedAd})! ¡Ahora confirma el intercambio!");
        ...
        if (await CheckCloneChangedOffer(token).ConfigureAwait(false))
        {
            ...
            if (!await CheckCloneChangedOffer(token).ConfigureAwait(false))
            {
                Log("El compañero de intercambio cambió su Pokémon ofrecido.");
                return (offered, PokeTradeResult.TrainerTooSlow);
            }
        }
    ...
    private async Task<PA8> ApplyAutoOT(PA8 toSend, TradePartnerLA tradePartner, SAV8LA sav, CancellationToken token)
    {
        ...
        {
            Log("No se pueden aplicar los detalles del compañero: el manejador actual no puede ser un OT de otra generación.");
            return toSend;
        }
        ...
        {
            Log("El Pokémon no es válido, no se puede aplicar AutoOT.");
            return toSend;
        }
    }
