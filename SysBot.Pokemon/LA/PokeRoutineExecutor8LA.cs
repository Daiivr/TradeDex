...
public abstract class PokeRoutineExecutor8LA : PokeRoutineExecutor<PA8>
{
    ...
    public async Task<SAV8LA> IdentifyTrainer(CancellationToken token)
    {
        ...
        if (title != LegendsArceusID)
            throw new Exception($"{title} no es un título válido de Pokémon Legends: Arceus. ¿Está correcto tu modo?");

        // Verify the game version.
        var game_version = await SwitchConnection.GetGameInfo("version", token).ConfigureAwait(false);
        if (!game_version.SequenceEqual(LAGameVersion))
            throw new Exception($"La versión del juego no es compatible. Se esperaba la versión {LAGameVersion}, y la versión actual es {game_version}.");
