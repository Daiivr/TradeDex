...
public class PokeTradeBotLGPE(PokeTradeHub<PB7> Hub, PokeBotState Config) : PokeRoutineExecutor7LGPE(Config), ICountBot, ITradeBot
{
    ...
    public override async Task MainLoop(CancellationToken token)
    {
        try
        {
            ...
            OnConnectionSuccess();
            Log($"Iniciando el bucle principal de {nameof(PokeTradeBotLGPE)}.");
        ...
        Log($"Finalizando el bucle de {nameof(PokeTradeBotLGPE)}.");
    ...
    private async Task DoTrades(SAV7b sav, CancellationToken token)
    {
        ...
        while (!token.IsCancellationRequested && Config.NextRoutineType == type)
        {
            ...
            Log($"Iniciando el siguiente intercambio {type}{tradetype}. Obteniendo datos...");
    ...
    private async Task<PokeTradeResult> PerformLinkCodeTrade(SAV7b sav, PokeTradeDetail<PB7> poke, CancellationToken token)
    {
        ...
        if (Hub.Config.Legality.UseTradePartnerInfo && !poke.IgnoreAutoOT)
        {
            ...
            {
                Log($"Aplicando AutoOT al Pokémon usando el OT del entrenador: {tradeDetails.OT}, TID: {tradeDetails.TID}, SID: {tradeDetails.SID}");
        ...
        Log($"Buscando al usuario {poke.Trainer.TrainerName}");
        ...
        while (await LGIsinwaitingScreen(token))
        {
            ...
            if (btimeout.ElapsedMilliseconds >= 45_000)
            {
                ...
                Log($"{poke.Trainer.TrainerName} no encontrado");
        ...
        Log($"{poke.Trainer.TrainerName} encontrado");
        ...
        if (tradepartnersav.OT != sav.OT)
        {
            ...
            Log($"Se encontró al compañero de intercambio: {tradepartnersav.OT}, TID7: {tid7}, SID7: {sid7}, Juego: {tradepartnersav.Version}");
        ...
        if (tradepartnersav2.OT != sav.OT)
        {
            ...
            string tid7 = displayTID.ToString("D6");
            string sid7 = displaySID.ToString("D4");
            Log($"Se encontró al compañero de intercambio: {tradepartnersav2.OT}, TID7: {tid7}, SID7: {sid7}");
            TradeProgressChanged?.Invoke(60);
        ...
        poke.SendNotification(this, "Tienes 15 segundos para seleccionar tu Pokémon de intercambio");
    ...
    private async Task<PokeTradeResult> ConfirmAndStartTrading(PokeTradeDetail<PB7> detail, int slot, CancellationToken token)
    {
        ...
        Log("Confirmando e iniciando el intercambio...");
        TradeProgressChanged?.Invoke(76);

        await Click(A, 3_000, token).ConfigureAwait(false);
        for (int i = 0; i < 10; i++)
        {
            if (BitConverter.ToUInt16(await SwitchConnection.ReadBytesMainAsync(ScreenOff, 2, token), 0) == Boxscreen
                || BitConverter.ToUInt16(await SwitchConnection.ReadBytesMainAsync(ScreenOff, 2, token), 0) == menuscreen)
        ...
        var tradeCounter = 0;
        Log("Revisando si se recibió un Pokémon en la Ranura 1");
        ...
        while (true)
        {
            ...
            if (tradeCounter >= Hub.Config.Trade.TradeConfiguration.TradeAnimationMaxDelaySeconds)
            {
                ...
                Log("No se detectó un cambio en la Ranura 1.");
                TradeProgressChanged?.Invoke(0);
    ...
    {
        detail.SendNotification(this, "Resalta el Pokémon en tu caja que deseas clonar, ¡hasta 4 a la vez! Tienes 5 segundos entre selecciones para moverte al siguiente Pokémon. (¡Los primeros 5 segundos comienzan ahora!). Si deseas clonar menos de 6, permanece en el mismo Pokémon hasta que comience el intercambio.");
        ...
        detail.SendNotification(this, $"Agregaste {(Species)offeredpbmc.Species} a la lista de clonación");
        ...
        detail.SendNotification(this, "Saliendo del intercambio para inyectar los clones, por favor vuelve a conectarte usando el mismo código de enlace.");
        ...
        Log($"Buscando al usuario {detail.Trainer.TrainerName}");
        TradeProgressChanged?.Invoke(48);
        ...
        while (await LGIsinwaitingScreen(token))
        {
            ...
            if (btimeout.ElapsedMilliseconds >= 45_000)
            {
                ...
                Log($"{detail.Trainer.TrainerName} no encontrado");
                TradeProgressChanged?.Invoke(0);
        ...
        Log($"{detail.Trainer.TrainerName} encontrado");
        TradeProgressChanged?.Invoke(53);
        ...
        if (tradepartnersav.OT != sav.OT)
        {
            Log($"Se encontró al compañero de intercambio: {tradepartnersav.OT}, TID: {tradepartnersav.DisplayTID}, SID: {tradepartnersav.DisplaySID}, Juego: {tradepartnersav.Version}");
            TradeProgressChanged?.Invoke(60);
        ...
        if (tradepartnersav2.OT != sav.OT)
        {
            Log($"Se encontró al compañero de intercambio: {tradepartnersav2.OT}, TID: {tradepartnersav2.DisplayTID}, SID: {tradepartnersav2.DisplaySID}");
            TradeProgressChanged?.Invoke(60);
        ...
        foreach (var t in clonelist)
        {
            ...
            detail.SendNotification(this, $"✅ Enviando {(Species)t.Species}. Tienes 15 segundos para seleccionar tu Pokémon de intercambio");
            Log("Esperando en la pantalla de intercambio...");
            TradeProgressChanged?.Invoke(70);

            await Task.Delay(10_000, token).ConfigureAwait(false);
            detail.SendNotification(this, "Te quedan 5 segundos para llegar a la pantalla de intercambio y no romper el intercambio");
            await Task.Delay(5_000, token);
    ...
    private async Task<PokeTradeResult> ProcessDumpTradeAsync(PokeTradeDetail<PB7> detail, CancellationToken token)
    {
        ...
        detail.SendNotification(this, offeredpbm, "Aquí está el Pokémon que me mostraste.");
        ...
        while (quicktime.ElapsedMilliseconds <= 30_000)
        {
            ...
            {
                detail.SendNotification(this, newofferedpbm, "Aquí está el Pokémon que me mostraste.");

                offeredpbm = newofferedpbm;
            }
        }

        detail.SendNotification(this, "¡El tiempo se acabó!");
        return PokeTradeResult.Success;
    }
    ...
    private async Task EnterLinkCodeLG(PokeTradeDetail<PB7> poke, CancellationToken token)
    {
        if (poke.LGPETradeCode == null || !poke.LGPETradeCode.Any())
        {
            ...
            Log($"Usando el código de intercambio predeterminado: {string.Join(", ", poke.LGPETradeCode)}");
        }
        else
        {
            Log($"Ingresando el código de intercambio: {string.Join(", ", poke.LGPETradeCode)}");
        ...
        {
            Log($"Ingresando pictograma {codePosition}/3: {pc}");
        ...
        Log("Finalizado el ingreso del código de intercambio");
    ...
    private async Task ExitTrade(bool unexpected, CancellationToken token)
    {
        ...
        {
            attempts++;
            Log($"Intento de salida #{attempts}");

            // Check if we've exceeded max attempts
            if (attempts > MAX_ATTEMPTS)
            {
                Log("No se pudo regresar al overworld después de múltiples intentos. Reiniciando el juego...");
                TradeProgressChanged?.Invoke(0);
            ...
            if (await IsOnOverworldStandard(token))
            {
                Log("Correctamente en el overworld.");
                return;
            }
        ...
        Log("Regresó correctamente al overworld.");
    }
...
#pragma warning restore CS8601 // Possible null reference assignment.
            ...
            if (tradelgpe.Valid)
            {
                Log("El Pokémon es válido, aplicando AutoOT.");
                return Task.FromResult<PB7?>(cln);
            }
            else
            {
                Log("El Pokémon no es válido, no se aplicará AutoOT.");
        ...
        {
            Log("No se encontraron detalles de intercambio para el OT del entrenador proporcionado.");
            return Task.FromResult<PB7?>(null);
        }
    }
    ...
    private static void ClearOTTrash(PB7 pokemon, TradeCodeStorage.TradeCodeDetails? tradeDetails)
    {
        ...
        {
            LogUtil.LogInfo("AutoOT", "Los detalles del intercambio o el OT son nulos. Omitiendo ClearOTTrash.");
            return;
        }
