...
public abstract class PokeRoutineExecutor7LGPE : PokeRoutineExecutor<PB7>
{
    ...
    public async Task<SAV7b> IdentifyTrainer(CancellationToken token)
    {
        ...
        if (title != LetsGoEeveeID && title != LetsGoPikachuID)
            throw new Exception($"{title} no es un título válido de Pokémon: Let's Go. ¿Está correcto tu modo?");

        var sav = await GetFakeTrainerSAV(token).ConfigureAwait(false);
        InitSaveData(sav);

        if (!IsValidTrainerData())
            throw new Exception("Los datos del entrenador no son válidos. Consulta la wiki de SysBot.NET para datos incorrectos o inexistentes.");
    ...
    public async Task WriteBoxPokemon(PB7 pk, int box, int slot, CancellationToken token)
    {
        ...
        var encData = new byte[pk.SIZE_PARTY];
        pk.WriteEncryptedDataParty(encData);
        await Connection.WriteBytesAsync(encData.AsSpan(0, StoredLength).ToArray(), (uint)slotofs, token);
        await Connection.WriteBytesAsync(encData.AsSpan(StoredLength).ToArray(), (uint)(slotofs + (ulong)StoredLength + 0x70), token);
    }
