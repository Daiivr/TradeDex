...
public abstract class PokeRoutineExecutor9PLZA(PokeBotState Config) : PokeRoutineExecutor<PA9>(Config)
{
    ...
    public async Task<SAV9ZA> IdentifyTrainer(CancellationToken token)
    {
        ...
        if (title is not PLZAID)
            throw new Exception($"ID de título incorrecto: {title}. Se esperaba el ID de título PLZA: {PLZAID}");

        var game_version = await SwitchConnection.GetGameInfo("version", token).ConfigureAwait(false);
        if (!game_version.SequenceEqual(PLZAGameVersion))
            throw new Exception($"La versión del juego no es compatible. Se esperaba la versión {PLZAGameVersion}, la versión actual es {game_version}.");
        ...
        if (!IsValidTrainerData())
        {
            ...
            throw new Exception("Consulta la wiki de SysBot.NET para más información.");
        }
    ...
    public async Task InitializeHardware(IBotStateSettings settings, CancellationToken token)
    {
        ...
        Log("Configurando esperas HID específicas de PLZA");
    ...
    public async Task<bool> IsConnected(CancellationToken token)
    {
        ...
        {
            Connection.Log("El socket se desconectó, no se puede comprobar el estado en línea");
            return false;
        }
        ...
        try
        {
            ...
            if (data.Length == 0)
            {
                Connection.Log("Se recibieron datos vacíos al comprobar el estado de la conexión");
                return false;
            }
        ...
        {
            Connection.LogError($"Error al comprobar el estado de la conexión: {ex.Message}");
            return false;
        }
    }
