    ...
    where T : PKM, new()
{
    ...
    public bool Remove(TradeEntry<T> detail)
    {
        lock (_sync)
        {
            LogUtil.LogInfo(nameof(TradeQueueInfo<T>), $"Eliminando a {detail.Trade.Trainer.TrainerName}");
    ...
    public QueueResultAdd AddToTradeQueue(TradeEntry<T> trade, ulong userID, bool allowMultiple = false, bool sudo = false)
    {
        lock (_sync)
        {
                ...
                if (existingEntries.Count > 0)
                {
                    ...
                    if (!allowMultiple)
                    {
                        LogUtil.LogInfo(nameof(TradeQueueInfo<T>),
                            $"Entrada duplicada bloqueada en la cola: El usuario {userID} ya tiene {existingEntries.Count} entrada(s) en la cola");
                    ...
                    {
                        LogUtil.LogInfo(nameof(TradeQueueInfo<T>),
                            $"Lote diferente bloqueado: El usuario {userID} intenta poner en cola el UniqueTradeID {trade.Trade.UniqueTradeID} " +
                            $"pero ya tiene el UniqueTradeID {existingEntries.First().UniqueTradeID}");
                        return QueueResultAdd.AlreadyInQueue;
                    }
                }
            }
            ...
            if (TradeExtensions<T>.IsItemBlocked(trade.Trade.TradeData))
            {
                ...
                var itemName = held > 0 ? GameInfo.GetStrings("en").Item[held] : "(ninguno)";
                LogUtil.LogInfo(nameof(TradeQueueInfo<T>),
                    $"Intercambio bloqueado para el usuario {userID}: el objeto '{itemName}' no está permitido");
            ...
            if (trade.Trade.BatchTrades != null && trade.Trade.BatchTrades.Count > 0)
            {
                for (int i = 0; i < trade.Trade.BatchTrades.Count; i++)
                {
                    if (TradeExtensions<T>.IsItemBlocked(trade.Trade.BatchTrades[i]))
                    {
                        ...
                        LogUtil.LogInfo(nameof(TradeQueueInfo<T>),
                            $"Intercambio por lote bloqueado para el usuario {userID}: el Pokémon #{i + 1} ({speciesName}) tiene el objeto '{itemName}', el cual no está permitido");
                        return QueueResultAdd.NotAllowedItem;
                    }
                }
            }
