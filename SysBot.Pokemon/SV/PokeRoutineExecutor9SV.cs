...
public abstract class PokeRoutineExecutor9SV : PokeRoutineExecutor<PK9>
{
    ...
    public async Task CleanExit(CancellationToken token)
    {
        ...
        Log("Desacoplando los controles al salir de la rutina.");
    ...
    public async Task<SAV9SV> IdentifyTrainer(CancellationToken token)
    {
        ...
        if (title is not (ScarletID or VioletID))
            throw new Exception($"{title} no es un título válido de SV. ¿Está correcto tu modo?");

        // Verify the game version.
        var game_version = await SwitchConnection.GetGameInfo("version", token).ConfigureAwait(false);
        if (!game_version.SequenceEqual(SVGameVersion))
            throw new Exception($"La versión del juego no es compatible. Se esperaba la versión {SVGameVersion}, y la versión actual es {game_version}.");
    ...
    public async Task InitializeHardware(IBotStateSettings settings, CancellationToken token)
    {
        ...
        Log("Configurando esperas HID específicas de SV");
    ...
    public async Task<bool> IsConnectedOnline(ulong offset, CancellationToken token)
    {
        ...
        {
            Connection.Log("Socket desconectado, no se puede verificar el estado en línea");
            return false;
        }
        ...
        try
        {
            ...
            if (data.Length == 0)
            {
                Connection.Log("Se recibieron datos vacíos al verificar el estado de conexión");
                return false;
            }
        ...
        {
            Connection.LogError($"Error al verificar el estado de conexión: {ex.Message}");
            return false;
        }
    }
    ...
    public async Task StartGame(PokeTradeHubConfig config, CancellationToken token)
    {
        ...
        if (timing.AvoidSystemUpdate)
        {
            ...
            await Click(A, 1_000 + loadPro, token).ConfigureAwait(false); // Select "Start Software" + delay if Profile selection is needed
        }
        ...
        while (!await IsOnOverworldTitle(token).ConfigureAwait(false))
        {
            ...
            {
                Log("¡Aún no estamos en el juego, iniciando protocolo de rescate!");
                while (!await IsOnOverworldTitle(token).ConfigureAwait(false))
                {
