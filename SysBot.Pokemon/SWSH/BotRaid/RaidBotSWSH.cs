...
public class RaidBotSWSH(PokeBotState Config, PokeTradeHub<PK8> Hub) : PokeRoutineExecutor8SWSH(Config), ICountBot
{
    ...
    private readonly RaidSettings Settings = Hub.Config.RaidSWSH;
    ...
    public override async Task MainLoop(CancellationToken token)
    {
        ...
        {
            Log("El tiempo de espera debe estar entre 0 y 180 segundos.");
            return;
        }
        ...
        try
        {
            ...
            Log("Iniciando el ciclo principal de RaidBot.");
        ...
        Log($"Finalizando el ciclo de {nameof(RaidBotSWSH)}.");
        ...
        if(Settings.EchoPartyReady)
{
            ...
            EchoUtil.Echo($"¡El jugador {player + 1} está listo con {shiny}{(Species)dexno}{altformstr}{gender}!");
        }
    ...
    private async Task DeleteAddFriends(CancellationToken token)
    {
        ...
        {
            Log("Navegando de regreso para agregar amigos.");
    ...
    private async Task HostRaidAsync(int code, CancellationToken token)
    {
        ...
        Log($"Iniciando incursión para {(Species)raidBossSpecies}.");
        ...
        if (code >= 0)
        {
            ...
            Log("Confirmando el Código de Enlace.");
            await Click(A, 2_000, token).ConfigureAwait(false);
        }

        if (addFriends && !string.IsNullOrEmpty(Settings.FriendCode))
            EchoUtil.Echo($"¡Envía una solicitud de amistad al Código de Amigo **{Settings.FriendCode}** para unirte! Los amigos serán agregados después de esta incursión.");

        // Invite others, confirm Pokémon and wait
        Log("Invitando a otros a la incursión...");
        ...
        var linkcodemsg = code < 0 ? "sin Código de Enlace" : $"código **{code:0000 0000}**";
        ...
        EchoUtil.Echo($"La sala de incursión para {raiddescmsg} está abierta con {linkcodemsg}.");

        var timetowait = Settings.TimeToWait * 1_000;
        var timetojoinraid = 175_000 - timetowait;

        Log("Esperando al grupo de incursión...");
        ...
        await Task.Delay(1_000, token).ConfigureAwait(false);
        EchoUtil.Echo($"La incursión está comenzando ahora con {linkcodemsg}.");
        ...
        Log("Finalizando la rutina de incursión.");
    ...
    private async Task InnerLoop(CancellationToken token)
    {
        while (!token.IsCancellationRequested && Config.NextRoutineType == PokeRoutineType.RaidBot)
        {
            ...
            {
                if (Settings is { NumberFriendsToAdd: > 0, RaidsBetweenAddFriends: > 0 })
                    addFriends = (encounterCount - Settings.InitialRaidsToHost) % Settings.RaidsBetweenAddFriends == 0;
                if (Settings is { NumberFriendsToDelete: > 0, RaidsBetweenDeleteFriends: > 0 })
            ...
            Log($"La incursión del anfitrión {encounterCount} ha finalizado.");
    ...
    {
        Log("Reiniciando la incursión reiniciando el juego");
