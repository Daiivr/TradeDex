...
public abstract class PokeRoutineExecutor8SWSH(PokeBotState Config) : PokeRoutineExecutor<PK8>(Config)
{
    ...
    public async Task CleanExit(CancellationToken token)
    {
        ...
        Log("Desconectando los controles al salir de la rutina.");
    ...
    {
        // Verificar si botbase está en la versión correcta o posterior.
        await VerifyBotbaseVersion(token).ConfigureAwait(false);

        // Verificar el título para advertir si el modo es incorrecto.
        string title = await SwitchConnection.GetTitleID(token).ConfigureAwait(false);
        if (title is not (SwordID or ShieldID))
            throw new Exception($"{title} no es un título válido de SWSH. ¿Tu modo es el correcto?");

        // Verificar la versión del juego.
        var game_version = await SwitchConnection.GetGameInfo("version", token).ConfigureAwait(false);
        if (!game_version.SequenceEqual(SWSHGameVersion))
            throw new Exception($"La versión del juego no es compatible. Se esperaba la versión {SWSHGameVersion}, y la versión actual es {game_version}.");

        Log("Obteniendo los datos del entrenador de la consola anfitriona...");
        ...
        if (!IsValidTrainerData())
        {
            ...
            throw new Exception("Consulta la wiki de SysBot.NET (https://github.com/kwsch/SysBot.NET/wiki/Troubleshooting) para más información.");
        }

        if (await GetTextSpeed(token).ConfigureAwait(false) < TextSpeedOption.Fast)
            throw new Exception("La velocidad del texto debe estar configurada en RÁPIDO. Corrige esto para un funcionamiento adecuado.");
    ...
    {
        // Reabrir el juego si recibimos un soft-ban
        Log("¡Posible soft ban detectado, reabriendo el juego por si acaso!");
        await CloseGame(config, token).ConfigureAwait(false);
        await StartGame(config, token).ConfigureAwait(false);

        // En caso de estar soft-baneados, restablecer la marca de tiempo
    ...
    public async Task SetupBoxState(IDumper DumpSetting, CancellationToken token)
    {
        ...
        {
            Log("¡La casilla de destino está ocupada! Guardando el Pokémon encontrado allí...");
            DumpPokemon(DumpSetting.DumpFolder, "saved", existing);
        }

        Log("Limpiando la casilla de destino para iniciar el bot.");
        PK8 blank = new();
    ...
    public async Task StartGame(PokeTradeHubConfig config, CancellationToken token)
    {
        ...
        while (!await IsOnOverworldTitle(token).ConfigureAwait(false) && !await IsInBattle(token).ConfigureAwait(false))
        {
            ...
            {
                Log("¡Aún no estamos dentro del juego, iniciando protocolo de rescate!");
                while (!await IsOnOverworldTitle(token).ConfigureAwait(false) && !await IsInBattle(token).ConfigureAwait(false))
    ...
    {
        // Al igual que en generaciones anteriores, el juego usa una marca de tiempo Unix
        // para determinar cuánto dura el soft ban, y una vez levantado
        // el juego restablece el valor a 0 (1970/01/01 12:00 AM (UTC))
        Log("Soft ban detectado, eliminando restricción.");
