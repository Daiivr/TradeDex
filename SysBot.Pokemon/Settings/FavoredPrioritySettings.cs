...
public class FavoredPrioritySettings : IFavoredCPQSetting
{
    ...
    [Category(Operation), Description("Determina cómo se calcula la posición de inserción de los usuarios favorecidos. \"Ninguno\" evitará que se aplique cualquier favoritismo.")]
    public FavoredMode Mode { get; set; }

    [Browsable(false)]
    public bool EnableFavoritism
    {
        get => Mode != FavoredMode.None;
        set => Mode = value ? FavoredMode.Multiply : FavoredMode.None;
    }

    [Browsable(false)]
    public int SkipPercentage
    {
        get => (int)Math.Round((1 - Multiply) * 100);
        set => Multiply = 1 - Math.Clamp(value, 0, 100) / 100f;
    }

    [Browsable(false)]
    public int MinimumRegularUsersFirst
    {
        get => MinimumFreeAhead;
        set => MinimumFreeAhead = value;
    }

    [Category(Configure), Description("Multiplicar: Insertado después de (usuarios no favorecidos)*(multiplicar) usuarios no favorecidos. Configurar esto en 0.2 inserta después del 20% de los usuarios.")]
