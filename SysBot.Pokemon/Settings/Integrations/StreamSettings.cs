...
public class StreamSettings
{
    ...
    public void IdleAssets(PokeRoutineExecutorBase b)
    {
        ...
        try
        {
            ...
            {
                File.WriteAllText("estimatedTime.txt", L("Estimated time: 0 minutes"));
                File.WriteAllText("estimatedTimestamp.txt", "");
            }
            if (CreateOnDeck)
                File.WriteAllText("ondeck.txt", L("Waiting..."));
            if (CreateOnDeck2)
                File.WriteAllText("ondeck2.txt", L("Queue is empty!"));
            if (CreateUserList)
                File.WriteAllText("users.txt", L("None"));
            if (CreateUsersInQueue)
                File.WriteAllText("queuecount.txt", L("Users in Queue: 0"));
        }
        catch (Exception e)
        {
            LogUtil.LogError(e.Message, nameof(StreamSettings));
    ...
    public override string ToString() => L("Configuración de transmisión");
    ...
    private void GenerateBotConnection<T>(PokeRoutineExecutorBase b, PokeTradeDetail<T> detail) where T : PKM, new()
    {
        ...
        File.WriteAllText($"{file}.txt", L(name));
    }
    ...
    private void GenerateCompletedTrades<T>(PokeTradeHub<T> hub) where T : PKM, new()
    {
        ...
        File.WriteAllText("completed.txt", L(msg));
    }
    ...
    private void GenerateEstimatedTime<T>(PokeTradeHub<T> hub) where T : PKM, new()
    {
        ...
        File.WriteAllText("estimatedTime.txt", L(wait));
    ...
    private void GenerateUsersInQueue(int count)
    {
        ...
        File.WriteAllText("queuecount.txt", L(value));
    }
