...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider declaring as nullable.
    ...
    private void InitializeRecoveryService()
    {
        ...
        {
            LogUtil.LogInfo("El sistema de recuperación del bot está habilitado", "Recovery");
        }
    }
    ...
    private void AddTradeBotMonitors()
    {
        ...
        if (!Directory.Exists(path))
            LogUtil.LogError("Hub", "La carpeta de distribución no fue encontrada. ¡Por favor verifica que exista!");

        var pool = Hub.Ledy.Pool;
        if (!pool.Reload(Hub.Config.Folder.DistributeFolder))
            LogUtil.LogError("Hub", "¡Nada para distribuir en colas de intercambio vacías!");
    }
