    ...
    public static class SpecialRequests
    {
        ...
        private static void LogHeldItem<T>(T pk, int heldItem, PokeRoutineExecutor<T> caller) where T : PKM, new()
        {
            ...
            if (heldItem > 0 && heldItem < allitems.Length)
            {
                ...
                caller.Log($"Objeto equipado: {itemHeld}");
            }
            else
            {
                caller.Log($"El objeto equipado estaba fuera de los límites del arreglo, o no había ningún objeto: {heldItem}");
            }
        }
            ...
            bool hasHomeTracker, int rewardItem, int heldItem) where T : PKM, new()
        {
            ...
            {
                detail.SendNotification(caller, "La naturaleza solicitada no se encontró en la base de datos.");
                return SpecialTradeType.FailReturn;
            }

            pk.Nature = pk.StatAlignment = result;
            ApplyCommonModifications(ref pk, rewardItem);
        ...
        private static SpecialTradeType HandleItemRequest<T>(ref T pk, PokeRoutineExecutor<T> caller, PokeTradeDetail<T> detail) where T : PKM, new()
        {
            ...
            if (item < 0)
            {
                detail.SendNotification(caller, "La solicitud de objeto no es válida. Revisa la ortografía y la generación.");
                return SpecialTradeType.FailReturn;
            }
        ...
        private static SpecialTradeType HandleBallRequest<T>(ref T pk, PokeRoutineExecutor<T> caller, PokeTradeDetail<T> detail) where T : PKM, new()
        {
            ...
            if (ball < 0)
            {
                detail.SendNotification(caller, "La solicitud de Poké Ball no es válida. Revisa la ortografía y la generación.");
                return SpecialTradeType.FailReturn;
            }
            ...
            SaveFile sav) where T : PKM, new()
        {
            ...
            if (loaded == null)
            {
                detail.SendNotification(caller, "¡Esta no es una solicitud válida!");
                return SpecialTradeType.FailReturn;
            }
        ...
        private static bool CheckLegalityAfterModification<T>(T pkm, PokeRoutineExecutor<T> caller, PokeTradeDetail<T> detail) where T : PKM, new()
        {
            ...
            if (!la.Valid)
            {
                detail.SendNotification(caller, "Esta modificación hizo que el Pokémon fuera ilegal según las comprobaciones de legalidad de PKHeX. No se puede completar esta solicitud. El Pokémon será devuelto sin cambios.");
        ...
        {
            // Si el Pokémon no tiene Home Tracker, podemos proceder con los cambios de clon
            if (!hasHomeTracker)
                return true;

            // El Pokémon tiene Home Tracker: bloquear todos los cambios de clon por seguridad
            detail.SendNotification(caller, "No se pueden aplicar cambios de clon a este Pokémon. Tiene un Home Tracker, y modificarlo invalidaría dicho rastreador. Por favor usa un Pokémon sin Home Tracker.");
            caller.Log("Cambio de clon bloqueado: el Pokémon tiene Home Tracker. Las modificaciones invalidarían el rastreador.");
            return false;
        }
    }
}
