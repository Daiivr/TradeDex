...
public class SeedSearchResult(Z3SearchResult Type, ulong Seed, int FlawlessIVCount, SeedCheckResults Mode)
{
    ...
    public override string ToString()
    {
        return Type switch
        {
            Z3SearchResult.SeedMismatch => $"Seed encontrada, pero no es una coincidencia exacta {Seed:X16}",
    ...
    private IEnumerable<string> GetLines()
    {
        if (FlawlessIVCount >= 1)
            yield return $"IV perfectos: {FlawlessIVCount}";
        yield return "Las spreads se listan por cantidad de IV perfectos.";

        SeedSearchUtil.GetShinyFrames(Seed, out int[] frames, out uint[] type, out List<uint[,]> IVs, Mode);

        for (int i = 0; i < 3 && frames[i] != 0; i++)
        {
            var shinytype = type[i] == 1 ? "Estrella" : "Cuadrado";
